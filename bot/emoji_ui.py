...
import contextlib
import random
from typing import Optional, List, Tuple, TypeVar, TYPE_CHECKING
...
class EmojiUI:
    ...
    def __init__(self, bot: Haruka, allowed_emojis: Tuple[str, ...]) -> None:
        self.bot = bot
    ...
    def user_id(self, value: Optional[int]) -> None:
        if value is not None and self.__user_id is None:
            try:
    ...
    def check(self, payload: discord.RawReactionActionEvent) -> bool:
        result = payload.message_id == self.message.id and str(payload.emoji) in self.allowed_emojis
        if self.user_id is not None:
            result = result and payload.user_id == self.user_id
        else:
            result = result and payload.user_id != self.bot.user.id
...
class StackedNavigatorPagination(EmojiUI):
    ...
    def __init__(self, bot: Haruka, pages: List[discord.Embed], breakpoints: List[int]) -> None:
        self.pages = pages
...
class SelectMenu(EmojiUI):
    ...
    def __init__(self, bot: Haruka, message: discord.Message, args_count: int) -> None:
...
class YesNoSelection(EmojiUI):
    ...
    def __init__(self, bot: Haruka, message: discord.Message) -> None:
